...
bench_v1:
	...
	USE_FLASH_V1=true torchrun --nproc_per_node 1 bench.py \
		--ckpt_dir $(LLAMA_CHAT_MODEL_PATH) \
		--tokenizer_path ./tokenizer.model \
		--max_seq_len $(MAX_SEQ_LEN) --max_batch_size 6 >> ./logs/bench_v1.log 2>&1
...
debug_v1:
	...
	USE_FLASH_V1=true torchrun --nproc_per_node 1 bench.py \
		--ckpt_dir $(LLAMA_CHAT_MODEL_PATH) \
		--tokenizer_path ./tokenizer.model \
		--max_seq_len $(MAX_SEQ_LEN) --max_batch_size 6 >> ./logs/debug_v1.log 2>&1
...
bench_minimal:
	...
	USE_FLASH_MINIMAL=true torchrun --nproc_per_node 1 bench.py \
		--ckpt_dir $(LLAMA_CHAT_MODEL_PATH) \
		--tokenizer_path ./tokenizer.model \
		--max_seq_len $(MAX_SEQ_LEN) --max_batch_size 6 >> ./logs/bench_minimal.log 2>&1
...
debug_minimal:
	...
	USE_FLASH_MINIMAL=true torchrun --nproc_per_node 1 bench.py \
		--ckpt_dir $(LLAMA_CHAT_MODEL_PATH) \
		--tokenizer_path ./tokenizer.model \
		--max_seq_len $(MAX_SEQ_LEN) --max_batch_size 6 >> ./logs/debug_minimal.log 2>&1

ref:
	torchrun --nproc_per_node 1 bench.py \
		--ckpt_dir $(LLAMA_CHAT_MODEL_PATH) \
		--tokenizer_path ./tokenizer.model \
		--max_seq_len $(MAX_SEQ_LEN) --max_batch_size 6 > ./logs/bench_ref.log 2>&1